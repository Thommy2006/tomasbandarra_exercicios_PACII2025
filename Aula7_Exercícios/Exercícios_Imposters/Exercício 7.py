# Exercício 7 — Stub: criar utilizador com sucesso (POST /api/users)
...
curl -X PUT http://localhost:2525/imposters/8000 \
  ...
  -d '{
    ...
    "stubs": [
      ...
      {
        "predicates": [{"equals": {"method": "POST", "path": "/api/users", "headers": {"Content-Type": "application/json"}}}],
        "responses": [{"is": {"statusCode": 201, "headers": {"Content-Type": "application/json", "Location": "/api/users/123"}, "body": {"id": 123, "status": "created"}}}]
      }
    ]
  }'
...
curl -i -X POST http://localhost:8000/api/users \
  -H "Content-Type: application/json" \
  -d '{"email":"novo@example.com","name":"Novo"}'